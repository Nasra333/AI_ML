...
from utils import onSystemPromptChanged, userMessage, responseStream, askAI  # Assuming `askAI` is a utility function to interact with the AI model
...
def build_job_match_tab():
    with gr.Column():
        ...
        def on_match(url: str, input_type: str, resume_content, resume_file, history: list):
            onSystemPromptChanged(default_system_prompts.get(TAB_JOB_MATCH, "You are a job match assistant, who takes job description and candidate resume and provides a match score, key matching skills, gaps and suggestions, and a brief tailored summary."))
        ...
        match_btn.click(on_match, [job_desc_url, resume_input_type, resume_text, resume_file, chatbot], [job_desc_url, chatbot], queue=False).then(
            responseStream, chatbot, chatbot
        )
        clear.click(lambda: None, None, chatbot, queue=False)
        return {"chatbot": chatbot, "match_btn": match_btn, "clear": clear, "job_desc_url": job_desc_url, "resume_input_type": resume_input_type, "resume_text": resume_text, "resume_file": resume_file}
