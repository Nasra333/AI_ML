...
<<<<<<< HEAD:ASimpleAIChatBot
    history.append({"role": "assistant", "content": ""})
    if (selectedModel is openaiSelect):
        result: str = ""
        response = openai.chat.completions.create(
            model = "gpt-4o-mini", 
            messages=history, 
        ...
        for character in result:
            history[-1]['content'] += character
            yield history
    elif (selectedModel is claudeSelect):
        response = anthropic.messages.stream(
            model="claude-opus-4-1-20250805",
            max_tokens=1024,
            temperature=0.7,
            system= systemMessage if systemMessage is not None else "You are a comedian that tell jokes.",
            messages = history
        )
        result: str = ""
        with response as stream:
...
=======
    ...
    for character in result:
        history[-1]['content'] += character
        yield history
>>>>>>> 7b5fc262010076802785dc80ad42bf36ea6a6731:utils.py
